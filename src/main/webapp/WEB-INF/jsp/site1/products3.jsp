...
<script type="text/javascript">
    ...
    function goSearch(){
    	...
    	$( "#mainpage" ).load("/products/listaprodutos3?size="+pageSize+"&name="+name, function() {
    ...
    function changePageSize() {
      //$("#searchForm").submit();
    	...
    	$( "#mainpage" ).load("/products/listaprodutos3?size="+pageSize+(name.length >0 ? "&name="+name : ""), function() {
			  //alert( "Load was performed." );
			});
    }
	...
	function goPage(page){
		...
		var pagina = "/products/listaprodutos3?page="+page+"&size="+pageSize+(name.length >0 ? "&name="+name : "");
		//alert(pagina)
...
<%--           <tr th:each="tutorial : ${tutorials}"> --%>
          ...
          <c:forEach var="p" items="${listaprods}" varStatus="i">
               ...
               <tr>
               	   <td>${p.id}</td>
                   <td style="text-align: center;"><img src="${p.dadosImagem}" width="80px"></td>
                    <td>${p.name}</td>
                    <td>${p.referencia}</td>
                    <td>${p.novidade}</td>
                    ...
                    <td>${p.activo}</td>
                   <td>
                       <a href="#" onclick="edit(${p.id})" class="btn btn-primary" title="Editar este registo">
                   ...
                   <td>
                   	 <a href="#" onclick="edit(${p.id})" title="Editar este registo"
...
<%--     <nav aria-label="Pagination" th:if="${totalPages > 0}"> --%>
      ...
      <ul class="pagination justify-content-center">
        <li class="page-item <c:if test="${currentPage == 1}">disabled </c:if>">
          <a title="Primeira Pagina" href="#" onclick="goPage(1)">
          <<
          </a>
        </li>
        <li class="page-item font-weight-bold <c:if test="${currentPage == 1}">disabled </c:if>">
          <a title="Pagina anterior" href="#" onclick="goPage(${currentPage - 1})">Prev</a>
        </li>
        ...
        <li class="page-item font-weight-bold <c:if test="${currentPage == totalPages}">disabled</c:if>">
          <a title="Próxima Pagina" href="#" onclick="goPage(${currentPage +1})">Next</a>
        </li>
        
         <li class="page-item <c:if test="${currentPage == totalPages}">disabled</c:if>">
          <a title="Úlima Pagina" href="#" onclick="goPage('${totalPages})">
          >>
          </a>
        </li>
