...
function apagar(id, titulo) {
    ...
    $(document).ready(function () {
      $(".btn-delete").on("click", function (e) {
        ...
        $("#confirmText").html("Deseja eliminar o anuncio nï¿½mero  \<strong\>" + idAd + "\<\/strong\> com descriï¿½ï¿½o "+nameAd+"?");
...
<%--     <nav aria-label="Pagination" th:if="${totalPages > 0}"> --%>
      ...
      <ul class="pagination justify-content-center">
        <li id="li0" class="page-item <c:if test="${currentPage == 1}">disabled </c:if>">
          <a title="Primeira Pagina" href="#li0" <c:if test="${currentPage != 1}">onclick="goPagina(1, event)"</c:if>>
          <<
          </a>
        </li>
        <li id="li1" class="page-item font-weight-bold <c:if test="${currentPage == 1}">disabled </c:if>">
          <a title="Pagina anterior" href="#li0" <c:if test="${currentPage != 1}">onclick="goPagina(${currentPage - 1}, event)"</c:if>>Anterior</a>
        </li>
        ...
        <li id="li2" class="page-item disabled">
 		...
 		<li id="li3" class="page-item <c:if test="${page == currentPage}">active</c:if>"> 
          ...
          <a href="#" onclick="goPagina(${page}, event)" title="${page}">	<c:out value="${page}"/> </a>
          </c:forEach>
          
        </li>
        
        <li id="li4 class="page-item <c:if test="${currentPage + 2 < totalPages}">disabled </c:if>">
          <a class="page-link" <c:if test="${currentPage + 2 == totalPages}">href="#" </c:if>>...</a>
        </li>
        
        <li id="li5" class="page-item font-weight-bold <c:if test="${currentPage == totalPages}">disabled</c:if>">
          <a title="Próxima Pagina" href="#li5" <c:if test="${currentPage != totalPages}">onclick="goPagina(${currentPage +1}, event)"</c:if>>Próximo</a>
        </li>
        
         <li id="li6" class="page-item <c:if test="${currentPage == totalPages}">disabled</c:if>">
          <a title="última Pagina" href="#li5" <c:if test="${currentPage != totalPages}">onclick="goPagina(${totalPages}, event)"</c:if>>
          >>
          </a>
        </li>
